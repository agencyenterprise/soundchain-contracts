  ...
  describe("Stake contract", function () {
    describe("Staking rewards", () => {
      it("should be given proportionally to each user", async function () {
        ...
        await network.provider.send("hardhat_mine", [
          "0x" + blocksToActivation.toString(16),
        ]);

        await nextPhase(1250000);
        const user1Phase1Rewards = ethers.utils.formatEther(
          await stake.callStatic.getReward(user1.address)
        );
        expect(user1Phase1Rewards).to.equal("3059909.058043133632");

        await nextPhase(3125000);
        const user1Phase2Rewards = ethers.utils.formatEther(
          await stake.callStatic.getReward(user1.address)
        );
        expect(user1Phase2Rewards).to.equal("6884793.39165695852");

        await nextPhase(10000000);
        const user1Phase3Rewards = ethers.utils.formatEther(
          await stake.callStatic.getReward(user1.address)
        );
        expect(user1Phase3Rewards).to.equal("10709678.337252350224");

        await nextPhase(15000000);
        const user1Phase4Rewards = ethers.utils.formatEther(
          await stake.callStatic.getReward(user1.address)
        );
        ...
        await stake.updateReward();
        const user1Rewards = (
          await stake.callStatic.getBalanceOf(user1.address)
        ).map(ethers.utils.formatEther);
        const user2Rewards = (
          await stake.callStatic.getBalanceOf(user2.address)
        ).map(ethers.utils.formatEther);
        const user3Rewards = (
          await stake.callStatic.getBalanceOf(user3.address)
        ).map(ethers.utils.formatEther);
        ...
        const user1BalanceBefore = ethers.utils.formatEther(
          await token.connect(user1).balanceOf(user1.address)
        );
        const user2BalanceBefore = ethers.utils.formatEther(
          await token.connect(user2).balanceOf(user2.address)
        );
        const user3BalanceBefore = ethers.utils.formatEther(
          await token.connect(user3).balanceOf(user3.address)
        );
        ...
        const [user2StakedAmount] = await stake
          .connect(user2)
          .getBalanceOf(user2.address);
        await stake.connect(user2).withdrawStake(user2StakedAmount);
        await stake.connect(user2).withdrawRewards();

        const [user3StakedAmount] = await stake
          .connect(user3)
          .getBalanceOf(user3.address);
        await stake.connect(user3).withdrawStake(user3StakedAmount);
        await stake.connect(user3).withdrawRewards();

        const user1BalanceAfter = ethers.utils.formatEther(
          await token.connect(user1).balanceOf(user1.address)
        );
        const user2BalanceAfter = ethers.utils.formatEther(
          await token.connect(user2).balanceOf(user2.address)
        );
        const user3BalanceAfter = ethers.utils.formatEther(
          await token.connect(user3).balanceOf(user3.address)
        );
    ...
    describe("Stake calculation for different phases with just 1 user", () => {
      ...
      it("should not increase balance with rewards if there are no new blocks in network", async function () {
        ...
        await network.provider.send("hardhat_mine", [
          "0x" + blocksToMove.toString(16),
        ]);
        await stake.connect(user1).stake(transfer1m);
        const [, balance] = await stake.callStatic.getUpdatedBalanceOf(
          user1.address
        );
      ...
      it("x1 - should increase balance of user 1 (by 32.0 phase 1)", async function () {
        ...
        const [_, balance] = await stake.callStatic.getUpdatedBalanceOf(
          user1.address
        );
      ...
      it("x1 - should increase balance of user 1 (by 5.0 phase 3)", async function () {
        let blocksToMove =
          PHASE_TWO_BLOCK + 1250000 + 122 - (await getCurrentBlock());
        await network.provider.send("hardhat_mine", [
          "0x" + blocksToMove.toString(16),
        ]);
        await stake.connect(user1).stake(transfer1m);
      ...
      it("x1 - should increase balance of user 1 (by 4 phase 4)", async function () {
        let blocksToMove =
          PHASE_THREE_BLOCK + 1250000 + 122 - (await getCurrentBlock());
        await network.provider.send("hardhat_mine", [
          "0x" + blocksToMove.toString(16),
        ]);
        await stake.connect(user1).stake(transfer1m);
      ...
      describe("Stake calculation between phases", () => {
        it("should calculate balance for previous and current phase (Phase 1 - 1 block and Phase 2 - 1 block)", async function () {
          let blocksToMove =
            PHASE_ONE_BLOCK - 2 + 122 - (await getCurrentBlock());
          await network.provider.send("hardhat_mine", [
            "0x" + blocksToMove.toString(16),
          ]);
          await stake.connect(user1).stake(transfer1m);
          ...
          const [_, balance] = await stake.callStatic.getUpdatedBalanceOf(
            user1.address
          ); // callStatic doesn't create blocks
        ...
        it("should calculate balance for previous and current phase (Phase 2 - 1 block and Phase 3 - 1 block)", async function () {
          let blocksToMove =
            PHASE_TWO_BLOCK - 2 + 122 - (await getCurrentBlock());
          await network.provider.send("hardhat_mine", [
            "0x" + blocksToMove.toString(16),
          ]);
          await stake.connect(user1).stake(transfer1m);
        ...
        it("should calculate balance for previous and current phase (Phase 3 - 1 block and Phase 4 - 1 block)", async function () {
          let blocksToMove =
            PHASE_THREE_BLOCK - 2 + 122 - (await getCurrentBlock());
          await network.provider.send("hardhat_mine", [
            "0x" + blocksToMove.toString(16),
          ]);
          await stake.connect(user1).stake(transfer1m);
        ...
        it("should just calculate balance for blocks from phase 4 and not extra blocks ( Phase 4 - Last block and extra 34 blocks)", async function () {
          let blocksToMove =
            PHASE_FOUR_BLOCK - 2 + 122 - (await getCurrentBlock());
          await network.provider.send("hardhat_mine", [
            "0x" + blocksToMove.toString(16),
          ]);
          await stake.connect(user1).stake(transfer1m);
    ...
    describe("withdrawRewards", () => {
      ...
      it("should reduce all balances correctly", async () => {
        ...
        await network.provider.send("hardhat_mine", [
          "0x" + moveToPointZero.toString(16),
        ]);
        await network.provider.send("hardhat_mine", [
          "0x" + PHASE_ONE_BLOCK.toString(16),
        ]);
      ...
      it("should not calculate rewards for account without balance", async function () {
        ...
        await stake.connect(user2).stake(ethers.utils.parseEther("1"));
        const [user1StakedAmount] = await stake.callStatic.getBalanceOf(
          user1.address
        );
        await stake.connect(user1).withdrawStake(user1StakedAmount);
        await stake.connect(user1).withdrawRewards();
        ...
        const [, balanceUser1] = await stake.callStatic.getUpdatedBalanceOf(
          user1.address
        );
