...
describe("Staking", () => {
  let owner: SignerWithAddress,
    ...
    beforeEach(async () => {
      ...
      const StakingContract = await ethers.getContractFactory("LiquidityPoolRewards");
  ...
  describe("LP Stake contract", function () {
    ...
    describe("Stake calculation", () => {
      ...
      it("x1 - should increase balance of user 1 by 20", async function () {
        await stake.connect(user1).stake(transfer1m);
        await stake.connect(user2).stake(transfer1m);
        const balance = await stake.callStatic.getUpdatedBalanceOf(user1.address);
        console.log('Balance: ', balance);
        expect(ethers.utils.formatEther(balance)).to.eq('1000020.692308');
      });
